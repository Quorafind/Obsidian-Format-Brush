	...
	displayMacroSettings(): void {
		...
		settings.brushes.forEach((brush, index) => {
			...
			new ToggleComponent(toggleEl).setValue(brush.enable).onChange((value) => {
				settings.brushes[index] = {...settings.brushes[index], enable: value};
				this.applySettingsUpdate();
			});
			...
			brushNameEl.createEl('input', {
				...
				settings.brushes[index] = {...settings.brushes[index], name: target.value};
				this.applySettingsUpdate();
			});
			...
			insertPrefixEl.createEl('input', {
				...
				const target = evt.target as HTMLInputElement;
				settings.brushes[index] = {
					...settings.brushes[index],
					insert: {...settings.brushes[index].insert, prefix: target.value}
				};
				this.applySettingsUpdate();
			});
			...
			insertSuffixEl.createEl('input', {
				...
				settings.brushes[index] = {
					...settings.brushes[index],
					insert: {...settings.brushes[index].insert, suffix: target.value}
				};
				this.applySettingsUpdate();
			});
